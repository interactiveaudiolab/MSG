...
def main():
    ...
    os.mkdir(f'/home/noah/TestSet{config.source}Full')
    device = torch.device(f"cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    with torch.no_grad():
        for i, s in enumerate(config.separators):
            os.mkdir(f'/home/noah/TestSet{config.source}Full/{s}')
            os.mkdir(f'/home/noah/TestSet{config.source}Full/{s}/MSG')
            eval_set = EV.EvaluationSet(songs_dir=f'/home/boaz/datasets/DemucsDataset/all_separator_set16/test/{s}Data16/',sample_rate=config.sample_rate,source=config.source)
            for (noisy,ground_truth,song_name) in eval_set:
                ...
                sf.write(f'/home/noah/TestSet{config.source}Full/{s}/MSG/{song_name}',generated_signal.peak_normalize().audio_data.T,config.sample_rate)
