...
class MusicDataset(Dataset):
    ...
    def _fileSize(self):
        return librosa.load(self.filenames_clean[0], sr=self.sr)[0].shape[0]
    ...
    def __getitem__(self, idx):
        ...
        data, sr = librosa.load(dirty_filename, sr=self.sr)
        dirty_sample = data[wave_idx * self.sample_length:(wave_idx + 1) * self.sample_length]

        data, sr = librosa.load(clean_filename, sr=self.sr)
