from models.MelGAN import GeneratorMel, GeneratorMelMix, DiscriminatorMel, MultiSpecDiscriminator
...
class ModelFactory():
    ...
    def discriminator(self):
        if self.multi_disc:
            return DiscriminatorMel(self.config.num_D, self.config.ndf, self.config.n_layers_D, self.config.downsamp_factor), MultiSpecDiscriminator(self.config.n_mel_channels,self.config.splits)
        else:
