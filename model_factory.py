...
class ModelFactory():
    ...
    def generator(self):
        ...
        if self.config.model == "demucs":
            return Demucs([self.config.source],audio_channels=self.config.audio_channels,  segment_length=int(self.config.segment_duration * self.config.sample_rate), skip_cxn = self.config.skip_cxn, mixture=self.config.use_mix)
        else:
            raise ValueError('Invalid Model')
    def discriminator(self):
        if self.config.multi_disc:
            return DiscriminatorMel(self.config.num_D, self.config.ndf, self.config.n_layers_D, self.config.downsamp_factor), MultiSpecDiscriminator(self.config.n_mel_channels,self.config.splits, self.optim, self.config)
        else:
